  ...
  return (
    <div>
      <p className="text-xs text-muted">
        <Link to={`/spaces/${spaceId}/tasks`} className="no-underline">
          ← Back to tasks
        </Link>
      </p>
      <h1 className="text-lg font-semibold mt-1">
        <CopyButton text={taskCopyKey(task.key, task.title)} label={`task identifier ${taskCopyKey(task.key, task.title)}`} iconClassName="h-4 w-4" className="mr-1.5 align-middle" />
        <span className="font-mono text-sm text-muted">{task.key}</span> ·{" "}
        {task.title}
      </h1>
      {msg && (
        <p className="text-xs text-accent mt-1" role="status">
          {msg}
        </p>
      )}
      {err && (
        ...
        <div className="md:col-span-2">
          <div className="card p-4">
            ...
            {task.status && (
              <p className="mt-1">
                <StatusDot
                  ...
                  label={`${task.status.name} (${task.status.category})`}
                />
              </p>
            )}
            ...
            <p className="text-xs text-muted mt-2">
              Priority: <PriorityBadge priority={task.priority} /> · Assignee:{" "}
              {task.assignee?.display_name ?? "Unassigned"} · Due:{" "}
              {task.due_date ?? "—"}{" "}
              {task.is_blocked
                ? `· Blocked: ${task.blocker_reason || "no reason given"}`
                : ""}
            </p>
            <p className="text-xs text-muted mt-1">
              Created {fmtDateTime(task.created_at)} · Updated{" "}
              {fmtDateTime(task.updated_at)} · Completed:{" "}
              {fmtDateTime(task.completed_at)} · Archived:{" "}
              {task.archived_at ? fmtDateTime(task.archived_at) : "no"}
            </p>
            ...
            <div className="grid sm:grid-cols-2 gap-2 mt-2">
              <div>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    save({ task_id: task.id, status_id: e.target.value })
                  }
                >
                  {(statuses ?? []).map((s) => (
              ...
              <div>
                <label className="label" htmlFor="d-assignee">
                  Assignee
                </label>
                <Select
                  id="d-assignee"
                  ...
                  onChange={(e) =>
                    save(
                      e.target.value
                        ? { task_id: task.id, assignee_id: e.target.value }
                        : { task_id: task.id, clear_assignee: true },
                    )
                  }
                >
                  ...
                  {(members ?? []).map((m) => (
                    <option key={m.user_id} value={m.user_id}>
                      {(m.profile as unknown as { display_name?: string })
            ...
            {canEditOwn && (
              <div className="mt-2 flex flex-wrap gap-2 items-center justify-between">
                <Button
                  ...
                  onClick={() =>
                    save({ task_id: task.id, archive: !task.archived_at })
                  }
                ...
                <label className="text-xs flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) =>
                      save({ task_id: task.id, is_blocked: e.target.checked })
                    }
                  />{" "}
                  Blocked
                </label>
          ...
          <GitPanel
            task={task}
            canEdit={canEditOwn}
            onSave={(p) => save({ task_id: task.id, ...p })}
          />

          <CommentsPanel
            taskId={task.id}
            canComment={
              role === "owner" || role === "lead" || role === "member"
            }
          />
        </div>
      ...
      <div className="grid gap-2">
        ...
        <div className="grid sm:grid-cols-2 gap-2">
          <div>
            ...
            <Input value={v.git_branch} onChange={set("git_branch")} />
          </div>
          <div>
            <label className="label">PR URL</label>
            <Input value={v.pr_url} onChange={set("pr_url")} />
          </div>
        </div>
        <div>
          <label className="label">Blocker reason</label>
          <Input value={v.blocker_reason} onChange={set("blocker_reason")} />
        </div>
        <Button
          variant="primary"
          onClick={() => onSave(v)}
...
function GitPanel({
  ...
  task: import("../../lib/database.types").Task;
  canEdit: boolean;
  onSave: (p: {
    git_branch?: string;
  ...
  return (
    <div className="card p-4 mt-3">
      <h2 className="text-sm font-semibold">
        Git info{" "}
        <span className="text-muted font-normal">(optional, manual entry)</span>
      </h2>
      <div className="mt-2 text-sm space-y-1">
        <p>
          Branch:{" "}
          <code className="font-mono text-xs">{task.git_branch ?? "—"}</code>
          {task.git_branch && (
            <CopyButton variant="button" text={task.git_branch} label={`branch ${task.git_branch}`} />
          )}
        </p>
        <p>
          PR:{" "}
          {task.pr_url ? (
            <>
              ...
              >
                Open PR <ExternalIcon className="h-3.5 w-3.5" />
              </a>
              <CopyButton variant="button" text={task.pr_url} label={`PR URL ${task.pr_url}`} />
            </>
          ) : (
            "—"
          )}
            ...
            <Button
              onClick={() =>
                onSave({
                  ...
                  clear_branch: !branch ? true : undefined,
                  pr_url: pr || undefined,
                  clear_pr: !pr ? true : undefined,
                })
              }
              icon={<SaveIcon className="h-4 w-4" />}
            >
  ...
  return (
    <div className="card p-4 mt-3">
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted mt-1">No comments yet.</p>
      ) : (
      ...
      {canComment && (
        <form
          ...
          onSubmit={handleSubmit(async (v) => {
            setErr(null);
            try {
              await add.mutateAsync({ task_id: taskId, body: v.body });
              reset();
            } catch (e) {
              setErr((e as Error).message);
            }
        ...
        >
          <Field label="Add update" error={formState.errors.body?.message}>
            <Textarea
              rows={2}
              {...register("body")}
              placeholder="Progress, context, next steps…"
            />
          </Field>
          {err && (
            <p className="error-text" role="alert">
  ...
  return (
    <div className="card p-4">
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted mt-1">No activity.</p>
      ) : (
        <ul className="mt-2 space-y-2">
          {data.map((a) => (
            <li
